...
FUNCTION GastosView(db, oParent, nX, nY, nW, nH)
   ...
   oBrw:AddColumn(HColumn():New(L("GastosTipoDoc"), {|v,o| (v), TipoDocStr(o:aArray[o:nCurrent, 4])}, "C", 10, 0, .F., DT_CENTER))
   ...
   @ nX+270, nY+nH-55 BUTTON L("GastosBtnTogglePago") SIZE 80, 28 OF oParent ON CLICK {|| GastoTogglePago(db, @aData, oBrw)}
   @ nX+370, nY+nH-55 BUTTON L("GastosBtnPdf") SIZE 50, 28 OF oParent ON CLICK {|| ExportPdfGastos(db, aData)}
RETURN NIL
...
STATIC FUNCTION GastoEditar(db, aData, oBrw, nRow)
   ...
   IF nRow < 1 .OR. nRow > Len(aData)
      hwg_MsgInfo(L("GastosMsgSeleccione"), L("CommonAviso"))
      RETURN
   ENDIF
...
STATIC FUNCTION GastoEliminar(db, aData, oBrw)
   ...
   IF nRow < 1 .OR. nRow > Len(aData)
      ...
      RETURN
   ENDIF
   IF hwg_MsgYesNo(StrTran(L("GastosMsgEliminar"), "{1}", aData[nRow][2]), L("CommonConfirmar"))
...
STATIC FUNCTION ExportPdfGastos(db, aData)
   LOCAL aCols := { ;
      ...
      {L("GastosFecha"), 90, 3}, ;
      {L("GastosTipoDoc"), 80, 4}, ;
      {L("GastosProveedor"), 160, 5}, ;
      {L("GastosBase"), 80, 6, .T.}, ;
      {L("GastosTotal"), 80, 7, .T.}, ;
      {L("GastosPagado"), 50, 8, .T.} }
   LOCAL cPath := AbrirListadoPdf(db, L("GastosTitle"), aData, aCols)
   IF !Empty(cPath); hwg_MsgInfo(StrTran(L("GastosMsgPdfGenerado"), "{1}", cPath), L("CommonExportar")); ENDIF
RETURN NIL

STATIC FUNCTION TipoDocStr(nTipo)
   LOCAL aTipos := {L("GastosTipoDocFactura"), L("GastosTipoDocTicket"), L("GastosTipoDocRecibo"), L("GastosTipoDocDua"), L("GastosTipoDocOtro")}
   IF nTipo < 1 .OR. nTipo > Len(aTipos)
      RETURN L("GastosTipoDocOtro")
   ENDIF
   RETURN aTipos[nTipo + 1]

STATIC FUNCTION MedioPagoStr(nPago)
   LOCAL aPagos := {L("GastosMedioPagoEfectivo"), L("GastosMedioPagoTransferencia"), L("GastosMedioPagoTarjeta"), L("GastosMedioPagoDomiciliacion"), L("GastosMedioPagoOtro")}
   IF nPago < 1 .OR. nPago > Len(aPagos)
      RETURN L("GastosMedioPagoOtro")
   ENDIF
