...
STATIC FUNCTION CatEditDialog(db, nId)
   ...
   @ 140, 79 CHECKBOX lIvaDeducible CAPTION L("CategoriasIvaDeducible") SIZE 130, 26
   @ 100, 150 BUTTON L("CategoriasGuardar") SIZE 80, 28 ON CLICK {|| oDlg:Close()}
   @ 220, 150 BUTTON L("CategoriasCancelar") SIZE 80, 28 ON CLICK {|| lCancel := .T., oDlg:Close()}
   ACTIVATE DIALOG oDlg CENTER
