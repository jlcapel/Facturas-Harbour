...
FUNCTION GenerarModelo303(db, nEjercicio, nTrimestre)
   ...
   cDir := hb_GetEnv("HOME") + "/Facturas/modelos303"
   hb_DirBuild(cDir)
   cPath := cDir + "/" + AllTrim(cNif) + "_" + LTrim(Str(nEjercicio)) + "_" + LTrim(Str(nTrimestre)) + "T.303"
   cContenido := GenerarFichero303(nEjercicio, nTrimestre, cNif, cRazon, cSis, cNifDev, cVer, ;
...
STATIC FUNCTION GenerarFichero303(nEjer, nTrim, cNif, cRazon, cSis, cNifDev, cVer, ;
   nB21, nC21, nB10, nC10, nB04, nC04, nBEx, nBDed, nCDed, nTotDev, nTotDed, nRes)
   LOCAL cRes := ""
   cRes += GenEncab303(nEjer, nTrim, cNif, cRazon, cSis, cNifDev, cVer) + Chr(13) + Chr(10)
   cRes += GenP01000303(nEjer, nTrim, cNif, cRazon) + Chr(13) + Chr(10)
   cRes += GenP03000303(nB21, nC21, nB10, nC10, nB04, nC04, nBEx, nBDed, nCDed, nTotDev, nTotDed) + Chr(13) + Chr(10)
   cRes += GenP04000303(nTotDev, nTotDed, nRes) + Chr(13) + Chr(10)
   cRes += GenPie303(nEjer, nTrim) + Chr(13) + Chr(10)
   RETURN cRes

STATIC FUNCTION GenEncab303(nEjer, nTrim, cNif, cRazon, cSis, cNifDev, cVer)
   LOCAL cAux, cEnc
   cEnc := "<T3030" + PadL(LTrim(Str(nEjer)), 4, "0") + PadL(LTrim(Str(nTrim)), 2, "0") + "0000>"
   cAux := "0200000000>" + "03" + PadR(cVer, 5) + PadR(cNif, 9) + PadR(cSis, 40) + PadR(cNifDev, 9)
   cEnc += "<AUX>" + PadR(cAux, 213) + "</AUX>"
   RETURN cEnc

STATIC FUNCTION GenP01000303(nEjer, nTrim, cNif, cRazon)
...
STATIC FUNCTION GenP03000303(nB21, nC21, nB10, nC10, nB04, nC04, nBEx, nBDed, nCDed, nTotDev, nTotDed)
...
STATIC FUNCTION GenP04000303(nTotDev, nTotDed, nRes)
...
STATIC FUNCTION GenPie303(nEjer, nTrim)
